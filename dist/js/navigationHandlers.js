...
function addEventListenerToUserInfo(userInfoContainers) {
   userInfoContainers.forEach(function (element) {
      element.addEventListener("click", function () {
         ...
         active ? (settingsPopup.dataset.active = "false") : (settingsPopup.dataset.active = "true");
...
function handleNavResizeLoad() {
   ...
   if (document.documentElement.clientWidth < 987) {
      mainWrapper.querySelector("[data-role='page-sub-content']").dataset.tablet = "true";
      hideSideBar(sideBar, btnNav, btnClassName, toggleClassName);
   } else {
      if (!btnNav.classList.contains(btnClassName)) {
         mainWrapper.querySelector("[data-role='page-sub-content']").dataset.tablet = "false";
         showSideBar(sideBar, btnNav, btnClassName, toggleClassName);
      }
   }
}
...
const isInViewPortRightLeft = (el) => {
   ...
   const inView = rect.left >= 0 && rect.right >= 0 && rect.right <= document.documentElement.clientWidth && rect.left <= document.documentElement.clientWidth;
   return inView;
};
...
if (window.location.pathname == "/dist/index.html") {
   ...
   setInterval(function () {
      ...
      if (batteryIcon.className.includes("fa-battery-empty")) {
         ...
         if (counter == 1) {
            batteryIcon.className = "fas fa-battery-three-quarters txt-success battery-icon";
         ...
         if (counter == 3) {
            batteryIcon.className = "fas fa-battery-quarter txt-warning battery-icon";
      ...
      if (counter == 1) {
         calibrationIcon.className = "fas fa-check-double txt-success calibration-icon";
         date.setMonth(date.getMonth() + 5);
         const after5Months = date.toLocaleDateString();
         calibrationLabel.innerText = after5Months;
      } else if (counter == 2) {
         calibrationIcon.className = "fas fa-check-double txt-danger calibration-icon";
         calibrationLabel.innerText = todaysDate;
      } else if (counter == 3) {
         date.setMonth(date.getMonth() + 1);
         const afterAMonth = date.toLocaleDateString();
         calibrationIcon.className = "fas fa-check-double txt-warning calibration-icon";
      ...
      if (counter) {
         lossTempIcon.className = "fas fa-angle-double-down txt-danger loss-temp-icon";
      } else {
         lossTempIcon.className = "fas fa-angle-double-down txt-success loss-temp-icon";
      }
